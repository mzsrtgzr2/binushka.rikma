...
3. `.github/workflows/cursor-agent-issue.yml` runs Cursor on a new branch named `cursor/issue-<number>`.
4. If Cursor changes files, the workflow commits, pushes, and opens a pull request.
5. Vercel posts/builds the PR preview.
6. The workflow comments on the original issue with the PR link.
...
Authorized comments are limited to GitHub users whose comment `author_association` is `OWNER`, `MEMBER`, or `COLLABORATOR`. Bot comments are ignored to avoid workflow loops.
