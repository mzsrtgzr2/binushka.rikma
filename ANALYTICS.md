...
צפיות בעמוד נשלחות אוטומטית (`track_pageview`). לחיצות ופעולות משמעותיות נשלחות כאירועים מפורשים. Autocapture כבוי, כדי שצפייה בעמוד ולחיצה על מוצר לא ייספרו פעמיים.
...
- **פרטיות**: המשלוחים הם בישראל, ואין באנר הסכמה. אם יהיו גולשות מאירופה או מקליפורניה, צריך שער הסכמה (`opt_out_tracking_by_default`) לפני `mixpanel.init`. לא להפעיל Session Replay בלי החלטה נפרדת — Clarity הוא כלי ההקלטות.
