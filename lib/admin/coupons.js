...
// String literals on purpose. `path.join(repoRoot, coupons.DIR)` is opaque to
// the file tracer, which then bundles the whole checkout (images, Jekyll
// output, .git) and blows past the 250MB function limit.
function bundledCouponsDir() {
  return path.join(__dirname, '..', '..', '_coupons');
}

function localCouponsDir(env) {
  return path.join(localRoot(env), '_coupons');
}

async function listCouponFiles(env) {
  const target = writeTarget(env);
  if (target === 'local') {
    const dir = localCouponsDir(env);
...
async function saveCoupon(env, body) {
  ...
  if (target === 'local') {
    const full = path.join(localCouponsDir(env), `${coupon.code.toLowerCase()}.md`);
...
async function deleteCoupon(env, rawCode) {
  ...
  const target = writeTarget(env);
  if (target === 'local') {
    const full = path.join(localCouponsDir(env), `${code.toLowerCase()}.md`);
