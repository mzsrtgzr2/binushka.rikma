...
function resolveInsideRoot(env, relativePath) {
...
export async function listDir(env, dirPath) {
  if (localRoot(env)) {
    const full = resolveInsideRoot(env, dirPath);
...
export async function readFile(env, filePath) {
  if (localRoot(env)) {
    const full = resolveInsideRoot(env, filePath);
    return full && fs.existsSync(full) ? fs.readFileSync(full, 'utf8') : null;
  }
