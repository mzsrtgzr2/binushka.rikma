...
// Invisible marks that show up when a password is copied out of WhatsApp,
// mail, or a notes app. trim() does not remove them, so the value stored in
// the env and the value typed into the form can look identical and still differ.
...
function normalizeSecret(value) {
  ...
  if (text.length >= 2) {
    ...
    if ((first === '"' && last === '"') || (first === "'" && last === "'")) {
      ...
      if (inner) text = inner.normalize('NFC');
    }
  }
  return text;
}
