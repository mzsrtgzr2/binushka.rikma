 ...
 * site build does.
 *
 * Parsing and catalog building come from api/admin-store.js, which the cart and
...
import store from '../../api/admin-store.js';
...
export function catalogFiles(rawByFile) {
  ...
  const content = store.prettyCatalog(store.buildWorkshopCatalogFromRaw(rawBySlug));
  return CATALOG_FILES.map((path) => ({ path, content }));
}

/** Spots and the two flags a workshop can be flipped from the list with. */
export function applyStock(raw, { spots, registration_full, hide }) {
  return store.applyWorkshopStock(raw, { spots, registration_full, hide });
}
