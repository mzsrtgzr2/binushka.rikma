...
/** Coupon category scope: shop product categories plus workshops. */
...
function parseCategory(raw) {
  const value = String(raw == null ? '' : raw)
    ...
    .toLowerCase();
  if (!value) return '';
  return Object.prototype.hasOwnProperty.call(COUPON_CATEGORIES, value) ? value : null;
}

/** Whether a cart line belongs to a coupon category (shop category or workshops). */
function lineMatchesCategory(line, category) {
  if (!line || !category) return false;
  if (category === 'workshops') return line.kind === 'workshop';
...
function eligibleSubtotal(order, coupon) {
  ...
  for (const line of lines) {
    ...
    if (appliesTo === APPLIES_PRODUCT) {
      ...
      if (!lineMatchesCategory(line, coupon.category)) continue;
    }
