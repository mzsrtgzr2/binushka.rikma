...
/**
 * Site-relative paths are meaningless once a message leaves for someone's
 * inbox: there is no page for a browser to resolve them against, so an
 * `/images/...` thumbnail simply never loads. Anything the editor wrote as a
 * path has to become a full URL before it is sent.
 */
export function absolutize(html, baseUrl) {
  const base = String(baseUrl || '').replace(/\/+$/, '');
  if (!base) return html;

  // Single leading slash only, so `//cdn.example` and `https://` are left be.
  return String(html).replace(
    /\b(src|href)=(["'])\/(?!\/)([^"']*)\2/g,
    (match, attribute, quote, rest) => `${attribute}=${quote}${base}/${rest}${quote}`
  );
}

function toPlainText(markdown, baseUrl) {
  const base = String(baseUrl || '').replace(/\/+$/, '');
  const absolute = (url) => (base && url.startsWith('/') && !url.startsWith('//') ? base + url : url);

  return String(markdown || '')
    .replace(/!\[[^\]]*\]\([^)]*\)/g, '')
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, text, url) => `${text} (${absolute(url)})`)
    .replace(/^#{1,6}\s+/gm, '')
    .replace(/[*_`>]/g, '')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
...
export function renderIssueEmail({ issue, settings, issueUrl, unsubscribeUrl, baseUrl }) {
  const email = settings?.email || {};
  // Falls back to the issue's own URL so a caller that forgets baseUrl still
  // produces working links rather than broken ones.
  const base = baseUrl || issueUrl.replace(/\/newsletter\/.*$/, '');
  const bodyHtml = absolutize(markdownToHtml(issue.body), base);
  const thumbnail = issue.thumbnail ? absolutize(`src="${escapeHtml(issue.thumbnail)}"`, base) : '';
...
<tr><td align="center">
  <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;font-family:Arial,Helvetica,sans-serif;direction:rtl;text-align:right;">
    ...
    ${thumbnail ? `<tr><td style="padding:16px 28px 0 28px;"><img ${thumbnail} alt="" style="display:block;width:100%;height:auto;border-radius:8px;"></td></tr>` : ''}
...
export async function sendIssue({
  ...
  baseUrl,
  now = () => Date.now(),
  // Both are seams for the tests; production uses the Gmail defaults.
  ...
  try {
    for (let index = 0; index < recipients.length; index += 1) {
      ...
      const { html, text } = renderIssueEmail({ issue, settings, issueUrl, unsubscribeUrl, baseUrl });
      ...
      if (gapMs > 0 && index < recipients.length - 1) {
