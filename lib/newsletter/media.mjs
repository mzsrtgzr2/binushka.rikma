 ...
 *
 * Files land in `images/newsletter/<year-month>/` rather than under the issue's
 * slug: slugs are Hebrew, which makes for awkward URLs, and an image can be
 * uploaded before the issue it belongs to has been saved and named.
 */
...
function safeName(name, extension) {
  ...
  // an image that an already-sent issue still points at.
...
export function folderFor(date = new Date()) {
  ...
  return `images/newsletter/${year}-${month}`;
}

/** Turns an editor upload into a file ready for commitFiles, plus its URL. */
export function prepareUpload(upload, { now = () => new Date() } = {}) {
  ...
  const path = `${folderFor(now())}/${safeName(upload.filename || upload.name, extension)}`;
