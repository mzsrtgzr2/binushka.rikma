...
function isAllowedPaymentUrl(raw, env) {
  ...
  if (PAYMENT_HOSTS.has(host) || hostAllowedBySuffix(host, PAYMENT_HOST_SUFFIXES)) {
...
// (front matter `form_url`). Anything else fails the content test.
const PAYMENT_LINK_HOSTS = new Set(['pay.grow.link', 'meshulam.co.il', 'mrng.to']);
const PAYMENT_LINK_HOST_SUFFIXES = ['.greeninvoice.co.il', '.morning.co', '.meshulam.co.il'];
...
function isAllowedPaymentLink(raw) {
  ...
  if (url.username || url.password) return false;
  const host = url.hostname.toLowerCase();
  return PAYMENT_LINK_HOSTS.has(host) || hostAllowedBySuffix(host, PAYMENT_LINK_HOST_SUFFIXES);
}
