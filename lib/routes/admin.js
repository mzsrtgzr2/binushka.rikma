...
function loginLocation(next, kind, env) {
  const vercelEnv = pickEnv(env, 'VERCEL_ENV');
  const params = new URLSearchParams({ login: kind });
  if (vercelEnv && vercelEnv !== 'production') params.set('env', vercelEnv);
...
async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    if (body.action === 'login') {
      ...
      const form = auth.wantsRedirect(req);
      // A correct password has to get in even after earlier misses. The lock
      // only stops more guesses; otherwise trying the real password a few
      // times on one warm instance looks like the password is wrong.
      if (auth.passwordMatches(body.password, env)) {
        loginLimiter.reset(ip);
        const cookies = { 'Set-Cookie': auth.loginSetCookie(env, { secure }) };
        if (form) return redirect(res, loginLocation(next, 'ok', env), cookies);
        return json(res, 200, { ok: true }, cookies);
      }
      if (loginLimiter.isBlocked(ip)) {
        if (form) return redirect(res, loginLocation(next, 'locked', env));
        return json(res, 429, { error: 'יותר מדי ניסיונות. נסי שוב בעוד כמה דקות' });
      }
      loginLimiter(ip);
      if (form) return redirect(res, loginLocation(next, 'error', env));
