...
const { resolveMorningEnv, morningHosts, getMorningToken } = require('../morning/client');
...
const { newOrderId, signOrder } = require('../order-token');
const {
  ...
  postPaymentForm,
} = require('../checkout/form');

const checkoutLimiter = createRateLimiter({ windowMs: 60 * 1000, max: 20 });
// Coupon preview shares this route so Hobby stays under the 12-function cap
// (a dedicated /api/coupon handler would still be fine with one catch-all, but
// we keep preview on checkout to match the existing client contract).
...
async function handler(req, res) {
  ...
  // An amount coupon can still zero the payable total (e.g. full fixed
  // discount + pickup). Grow cannot open a ₪0 form, so stock is taken here
  // and the shopper goes straight to thanks. Percent coupons cannot be 100%.
